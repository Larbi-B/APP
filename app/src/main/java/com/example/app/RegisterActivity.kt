...
//  Click
    ...
    private fun showNameExistAlert(isNotValid: Boolean){
        binding.etFullname.error = if (isNotValid) "le champ ne doit pas être vide !" else null
    }

    private fun showTextMinimalAlert(isNotValid: Boolean, text: String){
        if (text == "Username")
            binding.etUsername.error = if (isNotValid) "doit contenir plus de 6 caractères !" else null
        else if (text == "Password")
            binding.etPassword.error = if (isNotValid) "doit contenir plus de 8 caractères !" else null
    }

    private fun showEmailValidAlert(isNotValid: Boolean){
        binding.etEmail.error = if (isNotValid) "Email non valide !" else null
    }

    private fun showPasswordConfirmAlert(isNotValid: Boolean){
        binding.etConfirmPassword.error = if (isNotValid) "Le mot de passe ne correspond pas !" else null
    }
    ...
    private fun registerUser(email:String, password: String){
        auth.createUserWithEmailAndPassword(email, password)
            .addOnCompleteListener(this){
                if (it.isSuccessful){
                    startActivity(Intent(this, LoginActivity::class.java))
                    Toast.makeText(this, "Inscription réussite !", Toast.LENGTH_SHORT).show()
                }else{
